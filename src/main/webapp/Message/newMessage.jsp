...
<br/>
	...
	<% ArrayList<MessageDTO> newMessageList = (ArrayList<MessageDTO>)session.getAttribute("newMessageList"); %>
	<% System.out.print("newMessageList : "+newMessageList); %>
 	<% artistDTO artistDTO = (artistDTO)session.getAttribute("artistDTO"); %> 
 	<% System.out.print("artistDTO  : "+artistDTO); %>
	...
	<ul class="list-group">
 		<% for(int i=0; i<newMessageList.size(); i++){
			MessageDTO message = newMessageList.get(i);%>
		<li class="list-group-item">
		<img src="<%= message.getArtist_image() %>" class="img-thumbnail" alt="Cinque Terre" width="30" height="30">
		<a onclick="ajax('<%=message.getDM_CODE()%>')"><%=message.getArtist_email()%> 님의 메세지</a></li>
		<%} %> 

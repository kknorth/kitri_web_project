...
public class Follow_Query {
		...
		="select f.*, a.artist_id from fm_follow f, fm_artist a where f.artist_follow_code=a.artist_code and f.follow_good_alive=1 and f.artist_code= ?";
		
		public static String FOLLOW_SEARCH_FOLLOWER
		="select f.*, a.artist_id from fm_follow f, fm_artist a where f.artist_code=a.artist_code and f.follow_good_alive=1 and f.artist_follow_code= ?";
